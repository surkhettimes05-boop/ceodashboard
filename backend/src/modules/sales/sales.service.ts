...
import { SalesChannel, InventoryMovementType, PaymentMethod } from '@prisma/client';
...
export class SalesService {
  static async getPaymentSummary(startDate: Date, endDate: Date, branchId?: string) {
    const sales = await prisma.sale.findMany({
      ...
      select: { sale_payments: { select: { payment_method: true, amount: true } } },
    });

    const byMethod = Object.values(PaymentMethod).reduce<Record<string, { amount: number; transactionCount: number }>>((summary, method) => {
    ...
    const grandTotal = Object.values(byMethod).reduce((total, method) => total + method.amount, 0);
    return {
      startDate,
      endDate,
      grandTotal,
  ...
  static async createSaleTransaction(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (actor?.role === 'CASHIER' && (!actor.branchId || actor.branchId !== input.branchId)) {
        throw new Error('Cashier can only create sales for their assigned branch.');
      }

      if (input.payments.some((payment) => payment.paymentMethod === PaymentMethod.CREDIT) && !input.customerId) {
        throw new Error('A registered customer is required for store credit sales.');
      }
      ...
      if (redemptionPoints > 0) {
        if (!input.customerId) throw new Error('A registered customer is required to redeem loyalty points.');
        const loyaltySettings = await LoyaltyService.getSettings(tx);
        const customer = await tx.customer.findUnique({ where: { id: input.customerId }, select: { loyalty_points_balance: true } });
        if (!customer) throw new Error('Customer not found');
        if (redemptionPoints < loyaltySettings.minimumRedeemPoints) throw new Error(`Minimum redemption is ${loyaltySettings.minimumRedeemPoints} points`);
        if (Number(customer.loyalty_points_balance) < redemptionPoints) throw new Error('Insufficient points balance');
        redemptionDiscount = new Decimal(redemptionPoints).times(loyaltySettings.pointsValue);
      ...
      const creditAmount = input.payments
        ...
        .reduce((amount, payment) => amount.plus(new Decimal(payment.amount)), new Decimal(0));
      ...
      if (!totalPaid.equals(totalAmount)) {
        throw new Error(`Payment mismatch! Total payments ($${totalPaid.toString()}) must equal final total ($${totalAmount.toString()})`);
      }
      ...
      const sale = await tx.sale.create({
        ...
        include: {
          sale_items: { include: { product: { select: { name: true, sku: true } } } },
          sale_payments: true,
        },
      });
      ...
      if (creditAmount.greaterThan(0)) {
        await tx.customer.update({
          ...
          data: { outstanding_balance: { increment: creditAmount.toNumber() } },
        });
      }
      ...
      // Debits for Payments
      for (const pay of input.payments) {
        const accountCode = AccountingService.resolvePaymentAccountCode(pay.paymentMethod);
      ...
      if (totalCogs.greaterThan(0)) {
        journalLines.push({ accountCode: '5010', debit: totalCogs.toNumber(), credit: 0 }); // Debit COGS
        journalLines.push({ accountCode: '1040', debit: 0, credit: totalCogs.toNumber() }); // Credit Inventory Asset
      }

      // Credit Revenue
      const revenueAccountCode = AccountingService.resolveRevenueAccountCode(String(input.channel || SalesChannel.RETAIL));
      journalLines.push({
      ...
      await AuditService.log({
        ...
        entity: 'Sale',
        entityId: sale.id,
        newValues: { saleNumber, totalAmount: totalAmount.toNumber(), channel: input.channel },
      }, tx);
      ...
      if (input.customerId) {
        ...
        const previousBalance = Number(customerBeforeLoyalty.loyalty_points_balance);
        if (redemptionPoints > 0) {
        ...
        const points = await LoyaltyService.calculatePoints(totalAmount.toNumber(), tx);
        if (points > 0) {
        ...
        const loyaltyBalance = await LoyaltyService.getLoyaltyBalance(input.customerId, tx);
        loyaltyInfo = {
          pointsEarned: points,
  ...
  static async voidSale(saleId: string, userId: string, reason: string) {
    ...
    if (!isSameDay) {
      throw new Error('Only same-day sales can be voided. Use return/refund for older sales.');
    }
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (journalEntry) {
        await AccountingService.reverseJournalEntryTx(tx, journalEntry.id, userId, `Void of sale ${sale.sale_number}: ${reason}`);
      }
